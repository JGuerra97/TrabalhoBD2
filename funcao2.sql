...
CREATE OR REPLACE FUNCTION relatorio_funcionario_equipe()
	...
	DECLARE
		cursorFuncionarioEquipe CURSOR FOR
			...
			ON equipes_funcionarios.funcionario_id = funcionario.id AND equipes_funcionarios.equipe_id = equipe.id;
	BEGIN
		...
		FOR linha IN cursorFuncionarioEquipe LOOP
			...
			ELSIF coalesce(linha.eq_func_eq_id, -1) <> -1 THEN --se o funcionário é parte da equipe e não é lider
				INSERT INTO log_funcionarios_equipes VALUES
